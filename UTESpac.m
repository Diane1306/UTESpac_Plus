...
% Created by: Derek Jensen and Eric Pardyjak (derek591@gmail.com)
% Modified by Diane Wang (tdwang@ucdavis.edu)
% Updated Date: 5 September 2025
% 
% About:
% 
...
% enter root folder where site* folders are located *******NEED TO BE MODIFIED FIRST!!!!!!******
c_path = pwd;
if contains(c_path, 'diane_wt')
    info.rootFolder = '/Users/diane_wt/Library/CloudStorage/Box-Box/Lab Library/iFireX/UTESpac_iFireX_corrected';
elseif contains(c_path, 'admin-dianew68')
    info.rootFolder = 'C:/Users/admin-dianew68/Box/Diane/code/UTESpac_FM';
end
...
% calculate dissipation rate using structure function (can be time consuming)
info.calcDissipation = false; 

% calculate extra statistics for transport efficiencies
info.storeExtraStats = false;
...
template.LiH2O = 'LiH2O_*'; % for use with Licor 7500.  WPL corrections applied  --  [mmol/mol] % should be mmol/m^3, noted by Diane
template.LiCO2 = 'LiCO2_*'; % for use with Licor 7500.  WPL corrections applied  --  [mmol/mol] % should be mmol/m^3, noted by Diane
...
for i = 1:size(dataFiles,1)
    try
        ...
        [rotatedSonicData, PFSonicData, output, dataInfo] = sonicRotation(output,data,sensorInfo,info,dataInfo,tableNames,PFinfo);
...
%         [output, raw] = fluxes(data, rotatedSonicData, info, output, sensorInfo,tableNames);
        [output, raw] = fluxes(data, rotatedSonicData, PFSonicData, info, output, sensorInfo,tableNames);
...
%%
% outputStruct = getData(info.rootFolder,'avgPer',5,'qualifier','LinDet');