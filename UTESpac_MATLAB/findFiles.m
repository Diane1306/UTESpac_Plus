...
sitesStruct = dir(fullfile(info.rootFolder,'site*'));
sitesStruct = sitesStruct([sitesStruct.isdir]); % bug fix
...
for ii = 1:length(headers)
    ...
    try
        ...
        error('Unable to load %s @ line %g. Check format! \n%s', currentHeaderFileName, err.stack(1).line, err.message); % bug fix
    end
    ...
    for jj = 1:length(tableFiles)
        ...
        % modified by Diane
        csvDates(jj) = datenum(str2double(tableFile(dateBeginLoc+1:dateBeginLoc+4)),str2double(tableFile(dateBeginLoc+5:dateBeginLoc+6)),... % year, month
            str2double(tableFile(dateBeginLoc+7:dateBeginLoc+8)),str2double(tableFile(dateBeginLoc+9:dateBeginLoc+10)),... % day, hour
            str2double(tableFile(dateBeginLoc+11:dateBeginLoc+12)),str2double(tableFile(dateBeginLoc+13:dateBeginLoc+14))); % minute, second
    ...
    if ii == 1
        ...
        % dataFiles = cell(dateEnd-dateBegin+1,length(headers));
    end
    for jj = 1:length(csvFiles)
        dataFiles{floor(csvDates(jj))-dateBegin,ii} = csvFiles{jj};
        % dataFiles{floor(csvDates(jj))-dateBegin+1,ii} = csvFiles{jj}; % bug fix
    end
end
...
dataFiles(all(cellfun(@isempty,dataFiles),2),:) = []; % bug fix
