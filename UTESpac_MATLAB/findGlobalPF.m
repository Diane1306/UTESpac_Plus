...
if info.PF.recalculateGlobalCoefficients || ~exist([info.rootFolder,filesep,info.siteFolder,filesep,'PFinfo.mat'],'file')
    ...
    % data = getData(info.rootFolder,'site',siteNum,'avgPer',15,'qualifier','LPF','rows',0); % modified avgPer to 15 on 2/24/2025
    data = getData(info.rootFolder,'site',siteNum,'avgPer',info.avgPer,'qualifier','LPF','rows',0); % modified avgPer to 30 on 8/16/2025
    ...
    for ii = 1:length(z)
        ...
        % modfied by Diane 11/4/2025
        u = stpDn(u,round(info.PF.avgPer/info.avgPer));
        v = stpDn(v,round(info.PF.avgPer/info.avgPer));
        w = stpDn(w,round(info.PF.avgPer/info.avgPer));
        t = stpDn(t,round(info.PF.avgPer/info.avgPer));
        spd = stpDn(spd,round(info.PF.avgPer/info.avgPer));   
        
        % check sonic manufacturere: bug fix
        if sensorInfo.u(ii,5)==1
            uDir = u;
            vDir = v;
