...
try
    ...
    if isfield(sensorInfo,'P')
        ...
        [~, Pindex] = min(abs(sensorInfo.P(:,3) - zRef));  % ***bug fix, the lines above overide zRef to first P height
        Ptble = sensorInfo.P(Pindex,1);
        Pcol  = sensorInfo.P(Pindex,2);
    ...
    if ~isfield(sensorInfo,'RH') && (isfield(sensorInfo,'irgaH2O') || isfield(sensorInfo,'KH2O')) 
        ...
        if isfield(sensorInfo,'irgaH2O')
            ...
            [~, H2Oindex] = min(abs(sensorInfo.irgaH2O(:,3) - zRef)); % bug fix to locate lowest sonic
            rhoH2O_tble = sensorInfo.irgaH2O(H2Oindex,1);
            rhoH2O_col  = sensorInfo.irgaH2O(H2Oindex,2);
        ...
        elseif isfield(sensorInfo,'KH2O')
            ...
            [~, KH2Oindex] = min(abs(sensorInfo.KH2O(:,3) - zRef)); % bug fix to locate lowest sonic
            rhoH2O_tble = sensorInfo.KH2O(KH2Oindex,1);
            rhoH2O_col  = sensorInfo.KH2O(KH2Oindex,2);
            rhovIRGA = data{1,rhoH2O_tble}(:,rhoH2O_col);
            rhov_t = data{1,rhoH2O_tble}(:,1);
            rhovIRGAavg = simpleAvg([rhovIRGA,rhov_t],info.avgPer);
        ...
        elseif isfield(sensorInfo,'LiH2O')
            ...
            [~, LiH2Oindex] = min(abs(sensorInfo.LiH2O(:,3) - zRef)); % bug fix to locate lowest sonic
            rhoH2O_tble = sensorInfo.LiH2O(LiH2Oindex,1);
            rhoH2O_col  = sensorInfo.LiH2O(LiH2Oindex,2);
...
%                 raw.w = nan(bp(end),numSonics); % w
                ...
                raw.spd = nan(bp(end),numSonics);  % raw wind speed
...
%                 raw.Theta_v_sonPrime = nan(bp(end),numSonics);
                ...
                if isfield(sensorInfo,'irgaH2O')
                    ...
                    raw.rhov = nan(bp(end),numH2OSensors); % H2O  (g/m^3)
                    raw.rhovPrime = nan(bp(end),numH2OSensors); % H2O'  (g/m^3)
                    raw.rhovextenalPrime = nan(bp(end),numH2OSensors); % WPL H2O'  (g/m^3)
                elseif isfield(sensorInfo,'LiH2O')
                ...
                if isfield(sensorInfo,'irgaCO2')
                    ...
                    raw.rhoCO2extenalPrime = nan(bp(end),numCO2Sensors); % external CO2'
        ...
        try
            ...
            wCol = sensorInfo.w(sensorInfo.w(:,3)==sonHeight,2); % bug fix
            TsCol = sensorInfo.Tson(sensorInfo.Tson(:,3)==sonHeight,2);
            PCol = sensorInfo.P(sensorInfo.P(:,3)==sonHeight,2);
            ...
            TsSpikeFlag = output.(spikeFlagTableName)(:,TsCol); % bug fix
            ...
            % find raw wind directions and speeds
            ...
            WD_raw = mod(atan2(-1*v_WD,u_WD)*180/pi+bearing,360);
            spd_raw = sqrt(u_WD.^2 + v_WD.^2); % bug fix
            ...
            if isfield(sensorInfo,'RH') && isfield(sensorInfo,'T') && any(sensorInfo.RH(:,3) == sonHeight)
                ...
                % compute specific humidity for further analysis: Diane
                if info.useTrefHMP
                ...
                if isempty(x)  % if all NaNs, use qRef (non-local)
                    ...
                    if info.useTrefHMP % Diane 11/25/2025
                        output.specificHum(:,end+1) = virtualThetaAvg;
                        output.specificHum(:,end+1) = rAvg;
                        output.specificHum(:,end+1) = rho_airmoistAvg;
                        output.specificHum(:,end+1) = rho_airdryAvg;
                        output.specificHum(:,end+1) = rho_H2OAvg;
                    else
                        output.specificHum(:,end+1) = qRefavgLocal.*nan;
                        output.specificHum(:,end+1) = qRefavgLocal.*nan;
                        output.specificHum(:,end+1) = qRefavgLocal.*nan;
                    ...
                    output.specificHumHeader{end+1} = sprintf('%g m: virtualThetaAvg(K)',sonHeight);
                    output.specificHumHeader{end+1} = sprintf('%g m: rAvg(g/kg)',sonHeight);
                    output.specificHumHeader{end+1} = sprintf('%g m: rho_airmoistAvg(kg/m^3)',sonHeight);
                    output.specificHumHeader{end+1} = sprintf('%g m: rho_airdryAvg(kg/m^3)',sonHeight);
                    output.specificHumHeader{end+1} = sprintf('%g m: rho_H2OAvg(kg/m^3)',sonHeight);
                end
...
%                     raw.w(bp(jj)+1:bp(jj+1),ii) = w(bp(jj)+1:bp(jj+1));
                    raw.WD(bp(jj)+1:bp(jj+1),ii) = WD_raw(bp(jj)+1:bp(jj+1));
                    raw.spd(bp(jj)+1:bp(jj+1),ii) = spd_raw(bp(jj)+1:bp(jj+1));
...
%                     raw.w_tilt_Prime(bp(jj)+1:bp(jj+1),ii) = w_tilt_P;
                ...
                numRVariables = 16; % 5 for fluxes, 5 between quantities (including CO2 and CO2+CO2_WPL)
                ...
                % find turbulent transport of TKE and fluxes
                numturbtrVariables = 7;
                turbtr(jj,2+(ii-1)*numturbtrVariables) = mean(wPF_P .* 1/2.*(uPF_P.^2 + vPF_P.^2+ wPF_P.^2), 'omitmissing'); turbtrHeader{2+(ii-1)*numturbtrVariables} = strcat(num2str(sonHeight),"m :mean(w'e')");
                if rotatedSonFlag(jj); turbtr(jj,2+(ii-1)*numturbtrVariables) = nan; end;

                turbtr(jj,3+(ii-1)*numturbtrVariables) = mean(wPF_P .* (uPF_P.*wPF_P), 'omitmissing'); turbtrHeader{3+(ii-1)*numturbtrVariables} = strcat(num2str(sonHeight),"m :mean(w'u'w')");
                if rotatedSonFlag(jj); turbtr(jj,3+(ii-1)*numturbtrVariables) = nan; end;

                turbtr(jj,4+(ii-1)*numturbtrVariables) = mean(wPF_P .* (Theta_v_sonP.*wPF_P), 'omitmissing'); turbtrHeader{4+(ii-1)*numturbtrVariables} = strcat(num2str(sonHeight),"m :mean(w'theta_v'w')");
                if rotatedSonFlag(jj); turbtr(jj,4+(ii-1)*numturbtrVariables) = nan; end;
                ...
                if info.useTrefHMP && isfield(sensorInfo,'RH') && isfield(sensorInfo,'T') && any(sensorInfo.RH(:,3) == sonHeight)
                    T0_L = virtualThetaAvg(jj); % modified by Diane 11/25/2025
                else
                    T0_L = TvirtRef_Kavg(jj); % modified by Diane 11/20/2025
                end
...
%                     raw.Theta_v_sonPrime(bp(jj)+1:bp(jj+1),ii) = Theta_v_sonP;
                ...
                if ~isempty(irgaH2O) || ~isempty(KH2O)
                    ...
                    % compute external H2O fluctuations induced by water vapor and temperature
                    if info.useTrefHMP && isfield(sensorInfo,'RH') && isfield(sensorInfo,'T') && any(sensorInfo.RH(:,3) == sonHeight)
                        rhov_externalp = Md./Mv.*(rho_H2OAvg(jj)./rho_airdryAvg(jj)).*(H2Op./1000) + rho_H2OAvg(jj).*(1+Md./Mv .* rho_H2OAvg(jj)./rho_airdryAvg(jj)).*T_air_sonP./Tavg(jj); % kg/m^3 following Detto and Katul 2007, Coded by Diane
                    else
                        rhov_externalp = Md./Mv.*(rhovAvg(jj)./rhodAvg(jj)).*(H2Op./1000) + rhovAvg(jj).*(1+Md./Mv .* rhovAvg(jj)./rhodAvg(jj)).*T_air_sonP./Tref_Kavg(jj); % kg/m^3 following Detto and Katul 2007, Coded by Diane
                    end
                    ...
                    eta(jj, 5+(ii-1)*numetaVariables) = find_eta(wPF_P, H2Op+rhov_externalp.*1e3); etaHeader{5+(ii-1)*numetaVariables} = strcat(num2str(sonHeight),'m :eta_wPFH2O_WPL');
                    ...
                    delta_flux_ctrb(jj, 5+(ii-1)*numDelta_flux_ctrbVariables) = find_delta_flux(wPF_P, H2Op+rhov_externalp.*1e3); delta_flux_ctrbHeader{5+(ii-1)*numDelta_flux_ctrbVariables} = strcat(num2str(sonHeight),'m :S_wPFH2O_WPL');
                    ...
                    delta_time_ctrb(jj, 5+(ii-1)*numDelta_time_ctrbVariables) = find_delta_time(wPF_P, H2Op+rhov_externalp.*1e3); delta_time_ctrbHeader{5+(ii-1)*numDelta_time_ctrbVariables} = strcat(num2str(sonHeight),'m :D_wPFH2O_WPL');
                    ...
                    if info.saveRawConditionedData
                        ...
                        raw.rhovextenalPrime(bp(jj)+1:bp(jj+1),H2OsensorNumber) = rhov_externalp.*1e3; % (g/m^3)
                    end
                    ...
                    numLHfluxVariables = 8;
                    LHflux(jj,2+(ii-1)*numLHfluxVariables) = Lv; LHfluxHeader{2+(ii-1)*numLHfluxVariables} = strcat(num2str(sonHeight),'m Lv(J/g)');
                    
                    LHflux(jj,3+(ii-1)*numLHfluxVariables) = E; LHfluxHeader{3+(ii-1)*numLHfluxVariables} = strcat(num2str(sonHeight),'m w'':E(g/m^2s)');
                    if unrotatedSonFlag(jj)||H2OFlag(jj); LHflux(jj,3+(ii-1)*numLHfluxVariables) = nan; end;
                    ...
                    if H2Otype % see: https://www.eol.ucar.edu/content/corrections-sensible-and-latent-heat-flux-measurements 
                        ...
                        LHflux(jj,8+(ii-1)*numLHfluxVariables) = Lv*E; LHfluxHeader{8+(ii-1)*numLHfluxVariables} = strcat(num2str(sonHeight),'m O2 no WPL,w'' (W/m^2)');
                        LHflux(jj,9+(ii-1)*numLHfluxVariables) = Lv*EPF; LHfluxHeader{9+(ii-1)*numLHfluxVariables} = strcat(num2str(sonHeight),'m O2 no WPL,wPF'' (W/m^2)');
                        
                        % create WPL headers to include O2 correction
                        LHfluxHeader{6+(ii-1)*numLHfluxVariables} = strcat(num2str(sonHeight),'m WPL and O2, w'' (W/m^2)');
                        LHfluxHeader{7+(ii-1)*numLHfluxVariables} = strcat(num2str(sonHeight),'m WPL and O2, wPF'' (W/m^2)');
                    end
                    
                    % apply WPL Corrections (E.C. by Marc Aubinet 97).  Headers are created above for KH2O and below for EC150
                    LHflux(jj,6+(ii-1)*numLHfluxVariables) = 1000*Lv*(1+Md/Mv*rhovAvg(jj)/rhodAvg(jj))*(E./1000+(rhovAvg(jj)/Tref_Kavg(jj))*kinSenFlux); 
                    if unrotatedSonFlag(jj)||H2OFlag(jj); LHflux(jj,6+(ii-1)*numLHfluxVariables) = nan; end;
                    
                    LHflux(jj,7+(ii-1)*numLHfluxVariables) = 1000*Lv*(1+Md/Mv*rhovAvg(jj)/rhodAvg(jj))*(EPF./1000+(rhovAvg(jj)/Tref_Kavg(jj))*kinSenFlux); 
                    if unrotatedSonFlag(jj)||H2OFlag(jj); LHflux(jj,7+(ii-1)*numLHfluxVariables) = nan; end;
                    
                    
                    if ~H2Otype  % find CO2 flux and create H2O WPL headers
                        
                        % create WPL, H2O headers 
                        LHfluxHeader{6+(ii-1)*numLHfluxVariables} = strcat(num2str(sonHeight),'m WPL, w'' (W/m^2)');
                        LHfluxHeader{7+(ii-1)*numLHfluxVariables} = strcat(num2str(sonHeight),'m WPL, wPF'' (W/m^2)');
                        ...
                        evapFlux = LHflux(jj,7+(ii-1)*numLHfluxVariables)/Lv/1000; % WPL, wPF'' (kg/m^2s)

                        % convert mg/m3 to ppm
                        ppm_CO2 = rho_CO2(bp(jj)+1:bp(jj+1)).*1000.*8.314 .*(thetaSonAir(bp(jj)+1:bp(jj+1))+ 273.15) ./ (44.01.*(Pson(bp(jj)+1:bp(jj+1)).*1000));
                        ppm_CO2avg = mean(ppm_CO2, 'omitmissing'); % ppm
                        % ppm_CO2p = nandetrend(ppm_CO2,info.detrendingFormat); % ppm

                        % compute external CO2 fluctuations induced by water vapor and temperature
                        if info.useTrefHMP && isfield(sensorInfo,'RH') && isfield(sensorInfo,'T') && any(sensorInfo.RH(:,3) == sonHeight)
                            rhoc_externalp = Md./Mv.*(rho_CO2avg./rho_airdryAvg(jj)).*(H2Op./1000) + rho_CO2avg.*(1+Md./Mv .* rho_H2OAvg(jj)./rho_airdryAvg(jj)).*T_air_sonP./Tavg(jj); % kg/m^3 following Detto and Katul 2007, Coded by Diane
                        else
                            rhoc_externalp = Md./Mv.*(rho_CO2avg./rhodAvg(jj)).*(H2Op./1000) + rho_CO2avg.*(1+Md./Mv .* rhovAvg(jj)./rhodAvg(jj)).*T_air_sonP./Tref_Kavg(jj); % kg/m^3 following Detto and Katul 2007, Coded by Diane
                        end
                        
                        numCO2fluxVariables = 5;
                        CO2flux(jj,2+(ii-1)*numCO2fluxVariables) = mean(wP.*rho_CO2p, 'omitmissing'); CO2fluxHeader{2+(ii-1)*numCO2fluxVariables} = strcat(num2str(sonHeight),'m w'':CO2(kg/m^2s)');
                        if unrotatedSonFlag(jj)||CO2Flag(jj); CO2flux(jj,2+(ii-1)*numCO2fluxVariables) = nan; end;
                        
                        CO2flux(jj,3+(ii-1)*numCO2fluxVariables) = mean(wPF_P.*rho_CO2p, 'omitmissing'); CO2fluxHeader{3+(ii-1)*numCO2fluxVariables} = strcat(num2str(sonHeight),'m wPF'':CO2(kg/m^2s)');
                        if rotatedSonFlag(jj)||CO2Flag(jj); CO2flux(jj,3+(ii-1)*numCO2fluxVariables) = nan; end;
                        ...
                        CO2flux(jj,4+(ii-1)*numCO2fluxVariables) = CO2flux(jj,3+(ii-1)*numCO2fluxVariables) + Md/Mv*(rho_CO2avg/rhodAvg(jj))*evapFlux + (1+Md/Mv*rhovAvg(jj)/rhodAvg(jj))*(rho_CO2avg/Tref_Kavg(jj))*kinSenFlux; CO2fluxHeader{4+(ii-1)*numCO2fluxVariables} = strcat(num2str(sonHeight),'m WPL,wPF'':CO2(mol/m^2s)');
                        
                        CO2flux(jj,5+(ii-1)*numCO2fluxVariables) = CO2flux(jj,3+(ii-1)*numCO2fluxVariables) + mean(wPF_P.*rhoc_externalp, 'omitmissing'); CO2fluxHeader{5+(ii-1)*numCO2fluxVariables} = strcat(num2str(sonHeight),'m: wPF''CO2_WPL''(m/s kg/m^3)'); % Modified by Diane
                        
                        CO2flux(jj,6+(ii-1)*numCO2fluxVariables) = ppm_CO2avg; CO2fluxHeader{6+(ii-1)*numCO2fluxVariables} = strcat(num2str(sonHeight),'m: CO2 (ppm)'); % Modified by Diane
                        ...
                        if info.saveRawConditionedData
                            ...
                            raw.rhoCO2(bp(jj)+1:bp(jj+1),CO2sensorNumber) = rho_CO2(bp(jj)+1:bp(jj+1)); % mg/m^3
                        end

                        % more flux correlation
                        fuw = uPF_P.*wPF_P; fCw = (rho_CO2p + rhoc_externalp).*wPF_P; indx = ~(isnan(fuw) | isnan(fCw));
                        ...
                        fCw = (rho_CO2p + rhoc_externalp).*wPF_P; fwt = Theta_v_sonP.*wPF_P; indx = ~(isnan(fCw) | isnan(fwt));
                        ...
                        indx = ~(isnan(wPF_P) | isnan(rho_CO2p+rhoc_externalp));
                        R_mat_temp = corrcoef(wPF_P(indx), rho_CO2p(indx)+rhoc_externalp(indx));
                        ...
                        fuw = uPF_P.*wPF_P; fCw = rho_CO2p.*wPF_P; indx = ~(isnan(fuw) | isnan(fCw));
                        R_mat_temp = corrcoef(fuw(indx), fCw(indx));
                        R(jj, 12+(ii-1)*numRVariables) = R_mat_temp(1,2); RHeader{12+(ii-1)*numRVariables} = strcat(num2str(sonHeight),'m :R_uPFwPF_wPFCO2');
                        if TsonFlag(jj) || rotatedSonFlag(jj); R(jj, 12+(ii-1)*numRVariables) = nan; end;
                        
                        fCw = rho_CO2p.*wPF_P; fwt = Theta_v_sonP.*wPF_P; indx = ~(isnan(fCw) | isnan(fwt));
                        R_mat_temp = corrcoef(fCw(indx), fwt(indx));
                        R(jj, 13+(ii-1)*numRVariables) = R_mat_temp(1,2); RHeader{13+(ii-1)*numRVariables} = strcat(num2str(sonHeight),'m :R_wPFCO2_wPFThetav');
                        if TsonFlag(jj) || rotatedSonFlag(jj); R(jj, 13+(ii-1)*numRVariables) = nan; end;
                        
                        indx = ~(isnan(wPF_P) | isnan(rho_CO2p));
                        R_mat_temp = corrcoef(wPF_P(indx), rho_CO2p(indx));
                        R(jj, 14+(ii-1)*numRVariables) = R_mat_temp(1,2); RHeader{14+(ii-1)*numRVariables} = strcat(num2str(sonHeight),'m :R_wPF_CO2');
                        if TsonFlag(jj) || rotatedSonFlag(jj); R(jj, 14+(ii-1)*numRVariables) = nan; end;
                        
                        % more R: H2O WPL
                        fuw = uPF_P.*wPF_P; fHw = (H2Op + rhov_externalp.*1e3).*wPF_P; indx = ~(isnan(fuw) | isnan(fHw));
                        R_mat_temp = corrcoef(fuw(indx), fHw(indx));
                        R(jj, 15+(ii-1)*numRVariables) = R_mat_temp(1,2); RHeader{15+(ii-1)*numRVariables} = strcat(num2str(sonHeight),'m :R_uPFwPF_wPFH2O_WPL');
                        if TsonFlag(jj) || rotatedSonFlag(jj); R(jj, 15+(ii-1)*numRVariables) = nan; end;
                        
                        fHw = (H2Op + rhov_externalp.*1e3).*wPF_P; fwt = Theta_v_sonP.*wPF_P; indx = ~(isnan(fHw) | isnan(fwt));
                        R_mat_temp = corrcoef(fHw(indx), fwt(indx));
                        R(jj, 16+(ii-1)*numRVariables) = R_mat_temp(1,2); RHeader{16+(ii-1)*numRVariables} = strcat(num2str(sonHeight),'m :R_wPFH2O_WPL_wPFThetav');
                        if TsonFlag(jj) || rotatedSonFlag(jj); R(jj, 16+(ii-1)*numRVariables) = nan; end;
                        
                        indx = ~(isnan(wPF_P) | isnan(H2Op + rhov_externalp.*1e3));
                        R_mat_temp = corrcoef(wPF_P(indx), H2Op(indx)+ rhov_externalp(indx).*1e3);
                        R(jj, 17+(ii-1)*numRVariables) = R_mat_temp(1,2); RHeader{17+(ii-1)*numRVariables} = strcat(num2str(sonHeight),'m :R_wPF_H2O_WPL');
                        if TsonFlag(jj) || rotatedSonFlag(jj); R(jj, 17+(ii-1)*numRVariables) = nan; end;
                        
                        % more transport efficiency
                        eta(jj, 6+(ii-1)*numetaVariables) = find_eta(wPF_P, rho_CO2p+rhoc_externalp); etaHeader{6+(ii-1)*numetaVariables} = strcat(num2str(sonHeight),'m :eta_wPFCO2_WPL');
                        if TsonFlag(jj) || rotatedSonFlag(jj); eta(jj, 6+(ii-1)*numetaVariables) = nan; end;
                        
                        eta(jj, 7+(ii-1)*numetaVariables) = find_eta(wPF_P, rho_CO2p); etaHeader{7+(ii-1)*numetaVariables} = strcat(num2str(sonHeight),'m :eta_wPFCO2');
                        if TsonFlag(jj) || rotatedSonFlag(jj); eta(jj, 7+(ii-1)*numetaVariables) = nan; end;
                        
                        delta_flux_ctrb(jj, 6+(ii-1)*numDelta_flux_ctrbVariables) = find_delta_flux(wPF_P, rho_CO2p+rhoc_externalp); delta_flux_ctrbHeader{6+(ii-1)*numDelta_flux_ctrbVariables} = strcat(num2str(sonHeight),'m :S_wPFCO2_WPL');
                        if TsonFlag(jj) || rotatedSonFlag(jj); delta_flux_ctrb(jj, 6+(ii-1)*numDelta_flux_ctrbVariables) = nan; end;
                        
                        delta_flux_ctrb(jj, 7+(ii-1)*numDelta_flux_ctrbVariables) = find_delta_flux(wPF_P, rho_CO2p); delta_flux_ctrbHeader{7+(ii-1)*numDelta_flux_ctrbVariables} = strcat(num2str(sonHeight),'m :S_wPFCO2');
                        if TsonFlag(jj) || rotatedSonFlag(jj); delta_flux_ctrb(jj, 7+(ii-1)*numDelta_flux_ctrbVariables) = nan; end;
                        
                        delta_time_ctrb(jj, 6+(ii-1)*numDelta_time_ctrbVariables) = find_delta_time(wPF_P, rho_CO2p+rhoc_externalp); delta_time_ctrbHeader{6+(ii-1)*numDelta_time_ctrbVariables} = strcat(num2str(sonHeight),'m :D_wPFCO2_WPL');
                        if TsonFlag(jj) || rotatedSonFlag(jj); delta_time_ctrb(jj, 6+(ii-1)*numDelta_time_ctrbVariables) = nan; end;
                        
                        delta_time_ctrb(jj, 7+(ii-1)*numDelta_time_ctrbVariables) = find_delta_time(wPF_P, rho_CO2p); delta_time_ctrbHeader{7+(ii-1)*numDelta_time_ctrbVariables} = strcat(num2str(sonHeight),'m :D_wPFCO2');
                        if TsonFlag(jj) || rotatedSonFlag(jj); delta_time_ctrb(jj, 7+(ii-1)*numDelta_time_ctrbVariables) = nan; end;
                        ...
                        skew(jj,7+(ii-1)*numSkewVariables) = skewness(H2Op+rhov_externalp.*1e3,0); skewHeader{7+(ii-1)*numSkewVariables} = strcat(num2str(sonHeight),'m :skew_H2O_WPL');
                        if rotatedSonFlag(jj); skew(jj,7+(ii-1)*numSkewVariables) = nan; end;
                        
                        skew(jj,8+(ii-1)*numSkewVariables) = skewness(rho_CO2p,0); skewHeader{8+(ii-1)*numSkewVariables} = strcat(num2str(sonHeight),'m :skew_CO2');
                        if rotatedSonFlag(jj); skew(jj,8+(ii-1)*numSkewVariables) = nan; end;
                        
                        skew(jj,9+(ii-1)*numSkewVariables) = skewness(rho_CO2p+rhoc_externalp,0); skewHeader{9+(ii-1)*numSkewVariables} = strcat(num2str(sonHeight),'m :skew_CO2_WPL');
                        if rotatedSonFlag(jj); skew(jj,9+(ii-1)*numSkewVariables) = nan; end;
                        ...
                        Flux_lat(jj,4+(ii-1)*numFLatVariables) = mean(uPF_P.*(H2Op+rhov_externalp.*1e3), 'omitmissing'); Flux_latHeader{4+(ii-1)*numFLatVariables} = strcat(num2str(sonHeight),"m :uPF'H2O_WPL'");
                        if rotatedSonFlag(jj); Flux_lat(jj,4+(ii-1)*numFLatVariables) = nan; end;
                        
                        Flux_lat(jj,5+(ii-1)*numFLatVariables) = mean(uPF_P.*rho_CO2p, 'omitmissing'); Flux_latHeader{5+(ii-1)*numFLatVariables} = strcat(num2str(sonHeight),"m :uPF'CO2'");
                        if rotatedSonFlag(jj); Flux_lat(jj,5+(ii-1)*numFLatVariables) = nan; end;
                        
                        Flux_lat(jj,6+(ii-1)*numFLatVariables) = mean(uPF_P.*(rho_CO2p+rhoc_externalp), 'omitmissing'); Flux_latHeader{6+(ii-1)*numFLatVariables} = strcat(num2str(sonHeight),"m :uPF'CO2_WPL'");
                        if rotatedSonFlag(jj); Flux_lat(jj,6+(ii-1)*numFLatVariables) = nan; end;

                        % more turbulent transport terms
                        turbtr(jj,5+(ii-1)*numturbtrVariables) = mean(wPF_P .* (H2Op.*wPF_P), 'omitmissing'); turbtrHeader{5+(ii-1)*numturbtrVariables} = strcat(num2str(sonHeight),"m :mean(w'H2O'w')");
                        if rotatedSonFlag(jj); turbtr(jj,5+(ii-1)*numturbtrVariables) = nan; end;
                        
                        turbtr(jj,6+(ii-1)*numturbtrVariables) = mean(wPF_P .* ((H2Op+rhov_externalp.*1e3).*wPF_P), 'omitmissing'); turbtrHeader{6+(ii-1)*numturbtrVariables} = strcat(num2str(sonHeight),"m :mean(w'H2O_WPL'w')");
                        if rotatedSonFlag(jj); turbtr(jj,6+(ii-1)*numturbtrVariables) = nan; end;

                        turbtr(jj,7+(ii-1)*numturbtrVariables) = mean(wPF_P .* (rho_CO2p.*wPF_P), 'omitmissing'); turbtrHeader{7+(ii-1)*numturbtrVariables} = strcat(num2str(sonHeight),"m :mean(w'CO2'w')");
                        if rotatedSonFlag(jj); turbtr(jj,7+(ii-1)*numturbtrVariables) = nan; end;

                        turbtr(jj,8+(ii-1)*numturbtrVariables) = mean(wPF_P .* ((rho_CO2p+rhoc_externalp).*wPF_P), 'omitmissing'); turbtrHeader{8+(ii-1)*numturbtrVariables} = strcat(num2str(sonHeight),"m :mean(w'CO2_WPL'w')");
                        if rotatedSonFlag(jj); turbtr(jj,8+(ii-1)*numturbtrVariables) = nan; end;
