...
tCol = find(input(1,:) > datenum(2000,1,1) & input(1,:) < datenum(2030,1,1),1,'last');
...
if isempty(tCol) || N > size(input,1)  || (max(diff(input(:, tCol))) - min(diff(input(:, tCol)))) > datenum(0,0,0,0,0,0.5) % modified by Diane
...
for i = 1:N
    AVG(i,:) = mean(input(bp(i)+1:bp(i+1),:),1,'omitmissing');
    ...
    if exist('WDpntr','var') && exist('WSpntr','var')        
        ...
        v = mean(WS.*sin(WD),1,'omitmissing');
        u = mean(WS.*cos(WD),1,'omitmissing');
