function [rotatedSonicData, PFSonicData, output, dataInfo] = sonicRotation(output,data,sensorInfo,info,dataInfo,tableNames,PFinfo)
...
if isfield(sensorInfo,'u')
    ...
    for ii = 1:numSonics
        try
            ...
            wCol = sensorInfo.w(sensorInfo.w(:,3)==sonHeight,2);
            ...
            if strcmp(info.PF.globalCalculation,'global') && exist('PFinfo','var') && isfield(PFinfo,sprintf('cm_%g',sonHeight*100))
                ...
                % fprintf('length(dateBins): %d',length(dateBins)) % By Diane 2025/08/25
                ...
                for jj = 1:length(dateBins)
                    ...
                    % fprintf('local dates span %s to %s',datestr(min(tAvg)), datestr(max(tAvg))) % By Diane 2025/08/25
            ...
            N = min(floor((ceil(t(end)) - floor(t(1))) / (info.avgPer/(24*60))), length(cosGamma)); % bug fix
            
            % initialize rotated data
            if ii == 1
                rotatedSonicData = nan(size(windPF));
                PFSonicData = nan(size(windPF));
            else
                rotatedSonicData(:,3*ii-2:3*ii) = nan(size(windPF,1),3);
                PFSonicData(:,3*ii-2:3*ii) = nan(size(windPF,1),3);
            end
            ...
            output.PFSonicHeader{1,3*ii-2} = sprintf('%gm:u',sonHeight); % bug fix
            output.PFSonicHeader{1,3*ii-1} = sprintf('%gm:v',sonHeight);
            output.PFSonicHeader{1,3*ii} = sprintf('%gm:w',sonHeight);
            ...
            for row = 1:N
                ...
                PFSonicData(I0:I1,3*ii-2:3*ii) = windPF(I0:I1,:);
    ...
    try
        ...
        temp = simpleAvg([PFSonicData t],info.avgPer);
        output.PFSonic = temp(:,1:end-1);
    catch err
