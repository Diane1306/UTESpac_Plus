...
% function is a modified version of detrend that allows for nans. Modifications marked with %------
...
%------------- Mark NaNs and temporarily remove them
N = numel(x);
nans = isnan(x);
x = x(~isnan(x));

%  if all nans, return all nans
if sum(nans)/N > 0.9;
    y = x*nan;
else
    %-------------
